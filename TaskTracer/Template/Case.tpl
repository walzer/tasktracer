...
<div id="TopNavMain" class="CaseMode">
  <a id="TopNavLogo" href="./" target="_top">{$Lang.ProductName}</a>
    {if  $ActionType eq 'OpenCase'}
        <span id="TopBCRId">{$Lang.OpenCase}</span>
    {else}
        <span id="TopBCRId">{$Lang.ViewCase} #{$CaseInfo.CaseID}</span>
    {/if}
</div>
...
{if $HasChildren == 1}{assign var="OpenButtonStatus" value = 'disabled="disabled"'}{/if}

<div id="ButtonList">
  <input type="button" class="ActionButton" accesskey="P" value="{$Lang.PreButton}" onclick="location.href='Case.php?CaseID={$PreCaseID}'" {$PreButtonStatus}/>
  <input type="button" class="ActionButton" accesskey="N" value="{$Lang.NextButton}" onclick="location.href='Case.php?CaseID={$NextCaseID}'" {$NextButtonStatus}/>
  <input type="button" class="ActionButton" accesskey="E" value="{$Lang.EditCaseButton}" onclick="xajax.$('ActionType').value='Edited';submitForm('CaseForm')" {$EditButtonStatus}}/>
  <input type="button" class="ActionButton" accesskey="C" value="{$Lang.CopyCaseButton}" onclick="xajax.$('ActionType').value='OpenCase';submitForm('CaseForm')" {$EditButtonStatus}}/>
  <!-- input type="button" class="ActionButton" accesskey="R" value="{$Lang.RunCaseButton}" {$RunCaseButtonStatus} onclick="openWindow('Result.php?ActionType=OpenResult&CaseID={$CaseInfo.CaseID}','OpenResult');"/ -->
  <input type="button" class="ActionButton" accesskey="T" value="{$Lang.OpenDevelop}" {$OpenButtonStatus} onclick="openWindow('Bug.php?ActionType=OpenBug&CaseID={$CaseInfo.CaseID}','OpenBug');"/>
  <input type="button" class="ActionButton" accesskey="D" value="{$Lang.OpenTest}" {$OpenButtonStatus} onclick="openWindow('Result.php?ActionType=OpenResult&CaseID={$CaseInfo.CaseID}','OpenResult');"/>
  <input type="button" class="ActionButton" accesskey="S" value="{$Lang.SaveButton}" name="SubmitButton" id="SubmitButton" onclick="this.disabled='disabled';NeedToConfirm=false;document.CaseForm.submit();" {$SaveButtonStatus}/>
</div>
    ...
    <dl style="line-height:17pt">
      ...
      <dd>
          {if $ActionType eq 'OpenCase' or $EditMode eq 'true'}
            <input type="text" id="CaseTitle" name="CaseTitle" class="MyInput RequiredField" value="{$CaseInfo.CaseTitle}" maxlength="150" size="100%"/>
          {else}
            <input type="text" id="CaseTitle" name="CaseTitle" class="MyInput ReadOnlyField" readonly=true value="{$CaseInfo.CaseTitle}" maxlength="150" size="100%"/>
          {/if}
      </dd>
    </dl>
    <dl style="line-height:17pt">
        ...
        <table style="width: 100%">
            <tr>
              ...
              <fieldset class="Normal FloatLeft" style="width: 95%">
                ...
                <!-- Walzer Add, 父级模块ID -->
                <dl>
                  <dt>{$Lang.CaseFields.ParentID}</dt>
                  <dd>
					{if $ActionType eq 'OpenCase' or $EditMode eq 'true'}
						<input id="ParentID" name="ParentID" type="text" value="{$CaseInfo.ParentID}" size="20" maxlength="5" AUTOCOMPLETE="OFF" />
 					{else}
						<input id="ParentID" name="ParentID" type="text" value="{$CaseInfo.ParentID}" size="20" maxlength="5" readonly=true class="ReadOnlyField" AUTOCOMPLETE="OFF" />
					{/if}
                  </dd>
                </dl>
                <dl>
...
{/literal}
	...
	<!-- Walzer Add 功能点的分值和完成度显示 -->
 	<div id="CaseScore">
 		<table style="width: 100%">
 			<tr>
 				<td sytle = "width: 98%" valign="top">
 					<fieldset class = "Normal FloatLeft" id = "CaseScoreInfo" style = "width: 98%">
 						<legend>{$Lang.ModuleScore}</legend>
 						<!-- 总分值 -->
 						<dl>
 							<dt>{$Lang.CaseFields.ScoreTotal}</dt>
 							<dd>{$CaseInfo.ScoreTotal}</dd>
 						</dl>
 						<!-- 完成百分比 -->
 						<dl>
 							<dt>{$Lang.CaseFields.ResolvedPercent}</dt>
 							<dd>{$CaseInfo.ResolvedPercent}%</dd>
 						</dl>
 						<!-- 已完成分值 -->
 						<dl>
 							<dt>{$Lang.CaseFields.ScoreResolved}</dt>
 							<dd>{$CaseInfo.ScoreResolved}</dd>
 						</dl>
 						<!-- 未完成分值 -->
 						<dl>
 							<dt>{$Lang.CaseFields.ScoreUnresolved}</dt>
 							<dd>{$CaseInfo.ScoreUnresolved}</dd>
 						</dl>
 					</fieldset>
 				</td>
 			</tr>
