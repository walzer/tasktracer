...
this.create=function(sParentId,sTag,sId,sName){var objParent=this.$(sParentId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);objElement.setAttribute('name',sName);if(objParent)
objParent.appendChild(objElement);}
this.insert=function(sBeforeId,sTag,sId,sName){var objSibling=this.$(sBeforeId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);objElement.setAttribute('name',sName);objSibling.parentNode.insertBefore(objElement,objSibling);}
this.insertAfter=function(sAfterId,sTag,sId,sName){var objSibling=this.$(sAfterId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);objElement.setAttribute('name',sName);objSibling.parentNode.insertBefore(objElement,objSibling.nextSibling);}
...
errorString+="\nYou have whitespace in your response.";if(r.responseText.match("^<script")){parent.location='Login.php';}else{if(xajaxDebug)alert(errorString);};document.body.style.cursor='default';if(xajaxStatusMessages==true)window.status='Invalid XML response error';}
}
